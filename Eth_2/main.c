...
// two devices:
uint8_t mymac[6] = {0x00,0x21,0x68,0x01,0x00,0x29}; 
uint8_t myip[4];
...
void Init_My_IP(char *ptrMyIP, char *prtMyIP_Str)
{
	...
	{
		temp_myip_str[i]=eeprom_read_byte((uint8_t*)i+ADDR_MYIP_EEP);
	...
	for(uint8_t index_MyIP=0; index_MyIP<4; index_MyIP++)
	{
		...
		{
			temp=(eeprom_read_byte((uint8_t*)i+ADDR_MYIP_EEP)-0x30);
...
uint16_t PrintWebpage (uint8_t *buf)
{
   ...
   for(uint8_t i=1; i<5; i++)
   {
	...
	strcat(buf_eth_http, http_start);
   ...
   while (1)
   {
      ...
      if ((buf[IP_PROTO_P] == IP_PROTO_TCP_V) && (buf[TCP_DST_PORT_H_P] == 0) && (buf[TCP_DST_PORT_L_P] == TCP_PORT))
      {
         ...
         Make_tcp_ack_with_data(buf, plen); // send data       
	  	...
	  	// udp interface:
      if((buf[UDP_DST_PORT_H_P] == (UDP_PORT>>8)) && (buf[UDP_DST_PORT_L_P] == (uint8_t)UDP_PORT))
	  {
        char Data_RX[]="Hello!";
