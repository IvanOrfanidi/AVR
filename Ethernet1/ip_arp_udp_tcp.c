...
void Make_udp_reply_from_request(uint8_t *buf, char *data, uint8_t datalen, uint16_t port)
{
   ...
   buf[UDP_SRC_PORT_L_P] = port & 0xff; //!1
...
void Make_tcp_ack_with_full_data(uint8_t *buf, uint16_t dlen)	// Flag Fin disable.
{
   uint16_t j;
   ...
   // because we keep no state information. We must therefore set
   // the fin here:
   buf[TCP_FLAG_P] = TCP_FLAG_ACK_V | TCP_FLAG_PUSH_V; //| TCP_FLAG_FIN_V; // Flag Fin disable.
