   ...
   extern void Init_ip_arp_udp_tcp(uint8_t *mymac,uint8_t *myip,uint16_t wwwp);
   //
   ...
   extern uint16_t Fill_tcp_data_len (uint8_t *buf, uint16_t pos, const char *s, uint8_t len);
   extern uint16_t Get_tcp_data_len(void);
   extern void Make_tcp_ack_from_any(uint8_t *buf);
   extern void Make_tcp_ack_with_data(uint8_t *buf,uint16_t dlen);
   extern void Make_tcp_ack_with_full_data(uint8_t *buf, uint16_t dlen);
